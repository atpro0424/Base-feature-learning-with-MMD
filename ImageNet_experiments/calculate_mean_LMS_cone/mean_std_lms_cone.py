...
def main():
    ...
    transform_imagenet = transforms.Compose([
            ...
            transforms.ToTensor(),
            ConeTransform(),
            # transforms.Normalize(mean=[0.5910, 0.5758, 0.5298], 
            #                      std=[0.2657, 0.2710, 0.2806]),
            # Mean: tensor([0.5910, 0.5758, 0.5298])
            # Std Dev: tensor([0.2657, 0.2710, 0.2806])
        ])
    train_dataset = datasets.ImageFolder(root=traindir, transform=transform_imagenet)
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=1024, shuffle=True)
    channel_sum, channel_squared_sum, num_batches = 0, 0, 0

    # Loop over the dataset
    for data, _ in tqdm(train_loader):
        # Assumes data is in (B, C, H, W) format, B=batch size, C=channels, H=height, W=width
