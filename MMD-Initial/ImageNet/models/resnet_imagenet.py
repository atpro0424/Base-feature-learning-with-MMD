...
class CosineLinear(nn.Module):
    ...
    def forward(self, input):       
        if self.bias is not None:
            ...
            out = F.linear(F.normalize(input,p=2,dim=1,eps=1e-8), F.normalize(concat_weight,p=2,dim=1,eps=1e-8)) # default: eps=1e-12
        else:
            out = F.linear(F.normalize(input,p=2,dim=1,eps=1e-8), F.normalize(self.weight,p=2,dim=1,eps=1e-8)) # default: eps=1e-12
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, conv0_flag=False, conv0_outchannels=4):
        ...
        if conv0_flag:
            self.conv0 = nn.Conv2d(3, conv0_outchannels, kernel_size=3, stride=1, padding=1,
                               bias=True)
            self.conv1 = Conv2d(self.conv0.out_channels, 64, kernel_size=7, stride=2, padding=3,
                               bias=False)
        else:
...
class ResNet18_StartAt_Layer4_1(nn.Module):
    def __init__(self, num_classes=None):
        ...
        if num_classes is not None:
            print('Changing cosine output layer to contain %d classes.' % num_classes)
            #self.model.fc = nn.Linear(512, num_classes)
            self.model.fc = CosineLinear(512, num_classes)
